-- This script contains the queries needed for each type of analysis. It doesn't use views
-- In contrast to the stored procedure version as it allows for more organized results with
-- ORDER BY clauses



-- Change over time analysis
-- Total sales, customers and quantity per month and year.
SELECT
...
SUM([Total Sales])
OVER (PARTITION BY YEAR([Order Date]) 
ORDER BY [Order Date]) AS [Running Total Sales],
AVG([Average Price])
OVER (PARTITION BY YEAR([Order Date])
ORDER BY [Order Date]) AS [Moving Average Price]
FROM
...
--Analyze yearlyy performance of products
...
[Current Sales],
AVG([Current Sales]) 
OVER(PARTITION BY [Product Name]) AS [Average Sales],
[Current Sales] - AVG([Current Sales])
OVER (PARTITION BY [Product Name]) AS [Diff in Average],
...
END [Average Change],
LAG([Current Sales])
OVER(PARTITION BY [Product Name] ORDER BY [Order Year]) AS [Previous Year Sales],
...
ORDER BY [Product Name],[Order Year];
